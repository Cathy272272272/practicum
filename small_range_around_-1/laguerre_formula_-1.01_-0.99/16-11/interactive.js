function start(x) {
	return ((((((((((1.0 + (-11.0 * x)) + (27.5 * (x * x))) + (-27.5 * ((x * x) * x))) + (13.75 * (((x * x) * x) * x))) + (-3.85 * ((((x * x) * x) * x) * x))) + (0.641667 * (((((x * x) * x) * x) * x) * x))) + (-0.065476 * ((((((x * x) * x) * x) * x) * x) * x))) + (0.004092 * (((((((x * x) * x) * x) * x) * x) * x) * x))) + (-0.000152 * ((((((((x * x) * x) * x) * x) * x) * x) * x) * x))) + (3e-06 * (((((((((x * x) * x) * x) * x) * x) * x) * x) * x) * x)));
}
function end(x) {
	return (((((-0.000152 * x) + 0.004092) * (math.pow(x, 4.0) * math.pow(x, 4.0))) + ((math.pow(x, 4.0) * (13.75 + (-3.85 * x))) + ((x * -11.0) + 1.0))) + (((x * x) * ((-27.5 * x) + 27.5)) + ((math.pow(x, 3.0) * math.pow(x, 3.0)) * ((0.641667 + (-0.065476 * x)) + (math.pow(x, 3.0) * (3e-06 * x))))));
}
