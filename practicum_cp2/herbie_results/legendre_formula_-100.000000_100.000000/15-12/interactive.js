function start(x) {
	return ((((((0.225586 + (-17.595703 * (x * x))) + (219.946289 * (((x * x) * x) * x))) + (-997.089844 * (((((x * x) * x) * x) * x) * x))) + (2029.790039 * (((((((x * x) * x) * x) * x) * x) * x) * x))) + (-1894.470703 * (((((((((x * x) * x) * x) * x) * x) * x) * x) * x) * x))) + (660.194336 * (((((((((((x * x) * x) * x) * x) * x) * x) * x) * x) * x) * x) * x)));
}
function end(x) {
	return ((((-1894.470703 + ((x * x) * 660.194336)) * (((x * x) * math.pow(x, 3.0)) * ((x * x) * math.pow(x, 3.0)))) + (0.225586 + ((x * -17.595703) * x))) + (((x * x) * (x * x)) * (((2029.790039 * x) * math.pow(x, 3.0)) + ((-997.089844 * (x * x)) + 219.946289))));
}
