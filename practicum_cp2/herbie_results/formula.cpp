...
string write(long double dp[128], int idx, string x[128], long double LB, long double UB){


    string name = "";
    name += "(FPCore (x) :name \"" + name  + to_string(idx) + "\" ";
    name += '\n';
    name += ":pre (and (>= x " + to_string(LB) + ") (<= x " + to_string(UB) + "))";
    name += '\n';
    string s = "";
    bool flag = false;
    for ( int i = 0; i <= idx; i++ ){
...
int main(int argc, char *argv[]){
    ...
    ofstream out;
    filename += to_string(LB) + "_" + to_string(UB) +  ".fpcore";
